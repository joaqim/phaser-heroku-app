...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    this.add
      .text(500, 300, "press anywhere to hop", { fontSize: "50px" })
      .setOrigin(0.5, 0.5);
    ...
    this.socket.on("remove player", (pSocket) => {
      let o: Square[] = this.opponents.filter((player: Square) => {
      ...
      if (this.player.y > 700)
        this.player.applyForce(
          new Phaser.Math.Vector2(
            0.025 - 0.05 * Math.random(),
            -0.05 - 0.125 * Math.random()
          )
        );
    });
  }
  ...
  updateState() {
    ...
    return () => {
      ...
      if (
        this.player &&
        (Math.abs(this.player.x - oldX) > 3 ||
          Math.abs(this.player.y - oldY) > 3)
      ) {
        let data = {
